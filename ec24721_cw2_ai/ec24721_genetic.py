...
class Net(nn.Module):
    ...
    def __init__(self, genome, D=3, K=4):
        ...
        input_features = D
        for gene in genome:
            neurons = gene.get("neurons")
            activation = gene.get("activation")
            dropout = gene.get("dropout", 0)
    ...
    for _ in range(size):
        ...
        for _ in range(blueprint["max_n_layers"]):
            layer = {
                ...
                "activation": random.choice(blueprint["activation"]),
...
def selection(population, fitnesses, num_parents):
    ...
    ranks = [len(fitnesses) - i for i in sorted_indices]
    total_rank = sum(ranks)
    probabilities = [r / total_rank for r in ranks]

    selected_indices = np.random.choice(
        sorted_indices, size=num_parents, replace=False, p=probabilities
    )
...
# feel free to add keyword arguments to this function as you see fit
def mutate(genome, blueprint, mutation_rate):
    """
    ...
    for layer in genome:
        if random.random() < mutation_rate:
            ...
            if random.random() < mutation_rate:
                layer["activation"] = random.choice(blueprint["activation"])

            if random.random() < mutation_rate:
                layer["dropout"] = max(
                    0.0, min(1.0, layer["dropout"] + random.uniform(-0.1, 0.1))
                )
    return genome
...
blueprint = {
    "max_n_layers": 2,  # Define the (maximum) number of layers in each neural network
    "neurons": [64, 128, 256, 512],  # Possible neuron counts per layer
    "activation": [
        nn.ReLU(),
        nn.Tanh(),
        nn.Tanh(),
    ...
    "dropout": [
        ...
        0.4,
        0.5,
    ],  # Possible dropout rates, including 'None' for no dropout
}
...
for generation in range(num_generations):
    ...
    while len(next_generation) < population_size:
        ...
        child = mutate(child, blueprint, 0.5)
